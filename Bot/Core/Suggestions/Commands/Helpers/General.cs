                ...
                if (!isInSuggestionsChannel)
                {
                    if (config.SuggestionsChannelId is null)
                    {
                        await context.RespondAsync(
                            GenericEmbeds.Error(title: "Suggestions Channel Not Set", message: "The suggestions channel has not been set up yet. Set it up with `/config set suggestions channel` or contact a server administrator."));
                        return false;
                    }
