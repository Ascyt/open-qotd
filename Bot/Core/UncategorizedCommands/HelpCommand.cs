...
namespace OpenQotd.Core.UncategorizedCommands
{
    public sealed class HelpCommand
    {
        ...
        public static async Task HelpAsync(CommandContext context,
            [Description("Which viewable OpenQOTD profile to view general information of.")][SlashAutoCompleteProvider<Profiles.AutoCompleteProviders.ViewableProfiles>] int? For=null)
        {
        ...
        public static async Task<DiscordMessageBuilder> GetHelpMessageWithProfileSelectAsync(DiscordGuild guild, DiscordMember member, int? profileId)
        {
            Config? config = profileId is null ? 
                (await Profiles.Api.TryGetSelectedOrDefaultConfigAsync(guild.Id, member.Id)).Item1 : 
                await Profiles.Api.TryGetConfigAsync(guild.Id, profileId.Value);

            DiscordMessageBuilder messageBuilder = await GetHelpMessageAsync(config, guild, member);

            Dictionary<int, string> viewableProfiles = await Profiles.AutoCompleteProviders.ViewableProfiles.GetViewableProfilesAsync(guild, member, null);
