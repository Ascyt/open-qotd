    ...
    internal class Helpers
    {
        ...
        public static void AddButtonsIfEnabled(Config config, Question? question, DiscordMessageBuilder messageBuilder)
        {
            ...
            if (config.EnableQotdShowInfoButton)
                buttons.Add(new(DiscordButtonStyle.Secondary, $"show-general-info/{config.ProfileId}/{question?.GuildDependentId ?? -1}", "𝐢"));
        ...
        public static async Task CreateThreadIfEnabled(SendQotdData d, DiscordMessage sentMessage, int? sentQuestionsCount)
        {
            ...
            await Core.Helpers.General.RetryOnRateLimitAsync(() => sentMessage.CreateThreadAsync(
